...
- **Intentional regulations cache (LLM grounding, by design):** The local `regulations_cache.json` is a deliberate design choice, not an incidental performance cache. `build_prompt_context()` injects the cached **verbatim** CFR text into every LLM system prompt, and the prompt instructs the model to ground its compliance reasoning in that supplied regulatory text rather than its own training knowledge. This keeps regulatory citations authoritative, auditable, and version-pinned (the cache records `ecfr_dates`), and avoids the model hallucinating or paraphrasing CFR language. Treat the cache as the single source of regulatory truth the LLM is told to pull from — do not bypass it by feeding the model un-grounded regulatory claims.
